...
class RenderMap:
    def __init__(self, screen_width, screen_height, directory):
        ...
        self.minimap_tile=[[None for _ in range(self.graphe.width*2)] for _ in range(self.graphe.height*2)]
        for i, line in enumerate(self.graphe.matrix_of_node):
            for y, node in enumerate(line):
                if node != None: 
                    self.minimap_tile[i*2][y*2]=self.minimap_picture
                    id, E, S=self.graphe.get_type_node(node)
                    if E : self.minimap_tile[i*2][y*2 +1]=self.minimap_picture
                    if S : self.minimap_tile[i*2 +1][y*2]=self.minimap_picture
    ...
    def load_map_wave(self):
        # coordonates of the current map but rounded this time to fit in slices
        with open(f"{self.directory}\\assets\\tiled_maps\\levels\\number_of_maps.txt") as f:
            choice=random.randint(1, int(f.readlines()[0]))
        tm = pytmx.util_pygame.load_pygame(f'{self.directory}\\assets\\tiled_maps\\levels\\{choice}.tmx')
        ti = tm.get_tile_image_by_gid
        for a, layer in enumerate(tm.visible_layers):
            if a == 0:
                if isinstance(layer, pytmx.TiledTileLayer):
                    for y, x, gid, in layer:
                        if ti(gid):
                            self.current_map[x][y] = pygame.transform.scale(ti(gid), (round(ti(gid).get_width()*self.zoom), round(ti(gid).get_height()*self.zoom)))
                        else:
                            self.current_map[x][y] = None
        
        self.current_map_objects={"wall":[], "ground":[], "ceilling":[], "platform":[], "spawn_player":(), "spawn_crab":[], "bot":{"platform_right":[], "platform_left":[], "platform_go_right":[], "platform_go_left":[]}}   
        self._load_objects_map(self.current_map_objects, 0, 0, tm)
        self.current_map_is_wave=True
