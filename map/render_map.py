...
class RenderMap:
        ...
        for i in range(len(self.matrix_picture)):
            ...
            for y in range(len(self.matrix_picture[i])):
                ...
                for i_ in range(len(self.matrix_picture[i][y])):
                    if self.matrix_picture[i][y][i_]["img"] not in (len(self.all_pic)-3,len(self.all_pic)-2, len(self.all_pic)-1):
    ...
    def load_map(self, node, i, z, empty=False):
        ...
        if node:
            ...
            self.matrix_picture[i][z].append({"x":z*self.map_generation.room_width+self.map_generation.tile_width,"y":i*self.map_generation.room_height+self.map_generation.tile_width,"img":len(self.all_pic)-3,"type_image":1})
