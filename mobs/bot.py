...
class Bot:
    ...
    def _get_distance_target_x(self, abs_bool=False):
        ...
        else: return self.mob.position[0] - self.target.position[0]
    ...
    def _is_behind(self):
        ...
        if self.target.direction=="right":
            return self._get_distance_target_x() < -self.mob.range_attack +30 
    ...
    def make_mouvement(self, collision):
        ...
        if cpl and y > 100 and not cprj and not cplj:
            pressed_left([self.mob], collision)
            left=True
        
        elif cpr and y > 100 and not cprj and not cplj:
        ...
        if random.randint(1,self.random_coefficient["anticipation"][1])==1 and ((self.target.direction=="right" and 0 < x < (self.mob.range_attack)*2) or (self.target.direction=="left" and 0 > x > (-self.mob.range_attack)*2)) and self._get_distance_target_y(abs_bool=True)<50:
        ...
        if not right and not left:   
            if self.is_jumping_platform == False:
                ...
                if self.random_coefficient["type_attack"][1]==0 or (self.random_coefficient["type_attack"][1]==1 and behind and self.direction_behind==self.target.direction) or \
                    ...
                    if x < -self.mob.range_attack +30 and not(self.mob.is_attacking and sol): 
                        pressed_right([self.mob], collision)
                        right=True
        ...
        # we dont want every mob to jump at the same time so we add randomness
        if y > 100 and not cprj and not cplj and not cpl and not cpr: 
